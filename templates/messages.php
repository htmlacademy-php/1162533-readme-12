            ...
            <ul class="messages__contacts-list tabs__list">
                <?php if (!empty($message_users)) : ?>
                    <?php foreach ($message_users as $message_user) : ?>
                        <li
                                class="messages__contacts-item
                                <?= !$message_user['was_read'] && $message_user['sender_id'] !== $user['id']
                                    ? 'messages__contacts-item--new'
                                    : '' ?>">
                            <a
                                    class="messages__contacts-tab tabs__item
                                <?= $active_dialog === $message_user['user_id']
                                        ? 'messages__contacts-tab--active tabs__item--active'
                                        : '' ?>"
                                    href="<?= utils_url_to('messages', ['user' => $message_user['user_id']]) ?>">
                                <div class="messages__avatar-wrapper">
                                    ...
                                    <?php if ($message_user['unreaded_messages_count']) : ?>
                                        ...
                                        ><?= htmlspecialchars($message_user['unreaded_messages_count']) ?></i>
                                    <?php endif; ?>
                                </div>
                                <div class="messages__info">
                                    ...
                                    <div class="messages__preview">
                                        ...
                                        <time class="messages__preview-time"
                                              datetime="<?= $message_user['last_message'] ?>">
        ...
        <?php if ($active_dialog) : ?>
                ...
                <?php if (!empty($message_list)) : ?>
                        ...
                        <?php foreach ($message_list as $message) : ?>
                                <li class="messages__item <?= $message['sender_id'] == $user['id']
                                    ? 'messages__item--my'
                                    : '' ?>">
                ...
                <div class="comments">
                    <form
                            ...
                            action="<?= utils_url_to('messages', ['user' => $active_dialog]) ?>"
                            method="post">
                        ...
                        <div
                            class="form__input-section <?= !empty($errors) && $errors['message']
